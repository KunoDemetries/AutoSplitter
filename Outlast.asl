...
update 
{
  ...
  // outlast starter, ik it doesn't work if you start from new game
    if ((vars.Checker1) && (current.xcoord != vars.cx || current.zcoord != vars.cz)) 
    {
        vars.starter = 1;
    }
  // For whistleblower starter
    if ((vars.Checker2) && (current.xcoord != vars.cx || current.zcoord != vars.cz)) 
    {
        vars.starter = 1;
    }
    ...
    if ((settings[("il")]) && (vars.Running == 0)) 
    {
        ...
        if ((vars.Checker1) && (current.xcoord < 3700) && (current.isLoading == 0) && (current.map == "Prison_Start")) 
        {
            vars.starter = true;
        }

        if ((vars.Checker1) && ((vars.ILStartingListZ.Contains(current.map) && (current.zcoord != vars.cz)) || (vars.Checker1) && (vars.ILStartingListX.Contains(current.map) && (current.xcoord != vars.cx))))
        {
            vars.Checker1 = false;
            vars.starter = true;
        }
