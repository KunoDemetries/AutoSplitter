...
init
{
   ...
   Func<int, string, IntPtr> scan = (offset, pattern) => {
    ...
    vars.LevelName = scan(0x3, "48 8d 0d ?? ?? ?? ?? e8 ?? ?? ?? ?? 40 8a d7");
    ...
    print(vars.CurMap.ToString("X"));
    print(vars.LevelName.ToString("X"));
    print(vars.FinalTime.ToString("X"));
    print(vars.GWorld.ToString("X"));
